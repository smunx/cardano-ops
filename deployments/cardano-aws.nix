...
let
  inherit (pkgs.lib)
    ...
    getAttrs groupBy' concatLists mapAttrsToList zipListsWith partition optionals head;
  ...
  securityGroups = {
    ...
    core-nodes = {
      ...
      rules = map (sg: {
        protocol = "tcp";
        fromPort = pkgs.globals.cardanoNodeLegacyPort;
        toPort = pkgs.globals.cardanoNodeLegacyPort;
        sourceGroup = sg;
      }) [ "byron-proxies" "relay-nodes" ];
    };
    ...
    byron-proxies = {
      nodes = filterAttrs (_: n: n.node.roles.isByronProxy or false) nodes;
      rules = map (sg: {
        protocol = "tcp";
        fromPort = pkgs.globals.cardanoNodeLegacyPort;
        toPort = pkgs.globals.cardanoNodeLegacyPort;
        sourceGroup = sg;
      }) [ "legacy-core-nodes" "legacy-relay-nodes" ]
      ++ map (sg: {
        protocol = "tcp";
        fromPort = pkgs.globals.cardanoNodePort;
        toPort = pkgs.globals.cardanoNodePort;
        sourceGroup = sg;
      }) [ "core-nodes" "relay-nodes" ];
    };

    legacy-core-nodes = {
      nodes = filterAttrs (_: n: n.node.roles.isCardanoLegacyCore or false) nodes;
      rules = map (sg: {
        protocol = "tcp";
        fromPort = pkgs.globals.cardanoNodeLegacyPort;
        toPort = pkgs.globals.cardanoNodeLegacyPort;
        sourceGroup = sg;
      }) [ "byron-proxies" "legacy-relay-nodes" ];
    };

    legacy-relay-nodes = {
      nodes = filterAttrs (_: n: n.node.roles.isCardanoLegacyRelay or false) nodes;
      rules = [{
        protocol = "tcp";
        fromPort = pkgs.globals.cardanoNodeLegacyPort;
        toPort = pkgs.globals.cardanoNodeLegacyPort;
